...
// ----------- VLOG stuff -----------------
    ...
    struct VLogSite {
        ...
        VLogSite *next() { return _next.load(std::memory_order_acquire); }

        const VLogSite *next() const { return _next.load(std::memory_order_acquire); }

        void set_next(VLogSite *next) { _next.store(next, std::memory_order_release); }
