...
static void* sender(void* arg) {
    ...
    for (int i = 0; i < FLAGS_batch; ++i) {
        kvs[i].first = flare::string_format("{}{}", FLAGS_key.c_str(), base_index + i);
        kvs[i].second = flare::string_format("{}{}", FLAGS_value.c_str(), base_index + i);
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < FLAGS_batch * FLAGS_thread_num; ++i) {
        if (!request.Set(flare::string_format("{}{}", FLAGS_key.c_str(), i),
                         flare::string_format("{}{}", FLAGS_value.c_str(), i),
