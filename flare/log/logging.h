...
#define FLARE_CHECK(condition)  \
    ...
    struct CheckOpString {
        ...
        // No destructor: if str_ is non-NULL, we're about to FLARE_LOG(FATAL),
...
#define FLARE_DEFINE_CHECK_OP_IMPL(name, op) \
  ...
  inline std::string* name##Impl(int v1, int v2, const char* exprtext) { \
    ...
    // yacc grammar.
    FLARE_DEFINE_CHECK_OP_IMPL(Check_EQ, ==)  // Compilation error with FLARE_CHECK_EQ(NULL, x)?
    FLARE_DEFINE_CHECK_OP_IMPL(Check_NE, !=)  // Use FLARE_CHECK(x == NULL) instead.
