    ...
    public:
        ...
        static ParallelChannelDone *Create(
            ...
            if (ndone != nchan) {
                ...
                FLARE_CHECK_EQ(ndone, done_index);
            }
            return d;
        }
        ...
        void Run() {
            ...
            if (ec == EPCHANFINISH) {
                ...
                FLARE_CHECK(ECANCELED == ec || ERPCTIMEDOUT == ec) << "ec=" << ec;
            }
        ...
        void OnComplete() {
                    ...
                    if (!sd->cntl.FailedInline()) {  // successful calls only.
                        if (sd->merger == nullptr) {
                            try {
                                ...
                                nfailed = _ndone;
                                _cntl->SetFailed(ERESPONSE, "{}", e.what());
                                break;
                            }
                        } else {
                            ...
                            switch (res) {
                                ...
                                case ResponseMerger::FAIL_ALL:
                                    ...
                                    _cntl->SetFailed(
                                            ERESPONSE,
                                            "Fail to merge response of channel[{}]", i);
                                    break;
                            }
                        }
                    }
            ...
            if (nfailed >= _fail_limit) {
                ...
                if (!_cntl->FailedInline()) {
                    ...
                    _cntl->SetFailed(unified_ec, "{}/{} channels failed, fail_limit={}",
                                     nfailed, _ndone, _fail_limit);
                    std::string str;
                    for (int i = 0; i < _ndone; ++i) {
                        Controller *sub_cntl = &sub_done(i)->cntl;
                        if (sub_cntl->FailedInline()) {
                            str = flare::string_format(" [C{}]", i);
                            _cntl->_error_text.append(str);
...
// Used in controller.cpp
    ...
    int ParallelChannel::AddChannel(ChannelBase *sub_channel,
        ...
        if (nullptr == sub_channel) {
            FLARE_LOG(ERROR) << "Param[sub_channel] is nullptr";
            return -1;
        }
    ...
    void ParallelChannel::Reset() {
        ...
        for (size_t i = 0; i < uniq_size; ++i) {
            FLARE_CHECK_EQ(_chans[i].ownership, OWNS_CHANNEL);
    ...
    void *ParallelChannel::RunDoneAndDestroy(void *arg) {
        ...
        FLARE_CHECK_EQ(0, fiber_token_unlock_and_destroy(cid));
        return nullptr;
    }
    ...
    void ParallelChannel::CallMethod(
        ...
        if (rc != 0) {
            ...
            if (!cntl->FailedInline()) {
                cntl->SetFailed(EINVAL, "Fail to lock call_id={}", cid.value);
            }
        ...
        for (int i = 0; i < nchan; ++i) {
            ...
            if (sub_chan.call_mapper != nullptr) {
                ...
                if (aps[i].is_skip()) {
                    ...
                    cntl->SetFailed(
                            EREQUEST, "CallMapper of channel[{}] returns Bad()", i);
                    goto FAIL;
                }
            } else {
        ...
        if (ndone <= 0) {
            cntl->SetFailed(ECANCELED, "Skipped all channels({})", nchan);
            goto FAIL;
        }
        ...
        if (cntl->timeout_ms() >= 0) {
            ...
            const int rc = fiber_timer_add(
                    &cntl->_timeout_id,
                    flare::time_point::from_unix_micros(cntl->_deadline_us).to_timespec(),
