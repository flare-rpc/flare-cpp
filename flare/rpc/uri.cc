...
#include "flare/rpc/details/http_parser.h"      // http_parser_parse_url
#include "flare/rpc/uri.h"                      // URI
...
// ignoring of scheme("http://")
    ...
    int ParseURL(const char *url,
        ...
        for (; true; ++p) {
            ...
            if (*p == ':') {
                ...
                if (!is_all_spaces(p + 1)) {
                    FLARE_LOG(ERROR) << "Invalid space in url=`" << url << '\'';
                    return -1;
                }
                break;
            }
    ...
    void URI::InitializeQueryMap() const {
        if (!_query_map.initialized()) {
            FLARE_CHECK_EQ(0, _query_map.init(QUERY_MAP_INITIAL_BUCKET));
        }
    ...
    void append_query(std::string *query_string,
        ...
        if (!query_string->empty() && flare::back_char(*query_string) != '?') {
