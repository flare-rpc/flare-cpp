...
namespace flare::rpc {
    ...
    public:
        ...
        void move_in_server_receiving_sock(SocketUniquePtr &ptr) {
            FLARE_CHECK(_cntl->_current_call.sending_sock == NULL);
        ...
        ControllerPrivateAccessor &set_remote_side(const flare::base::end_point &pt) {
            _cntl->_remote_side = pt;
            return *this;
        }

        ControllerPrivateAccessor &set_local_side(const flare::base::end_point &pt) {
    ...
    // utility only useable by flare developers.
    class RPCSender {
    public:
        virtual ~RPCSender() {}
