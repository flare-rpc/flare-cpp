...
// Options to encode a header
    ...
    public:
        ...
        void Encode(flare::cord_buf_appender *out, const Header &header,
                    const HPackOptions &options);

        void Encode(flare::cord_buf_appender *out, const Header &header) { return Encode(out, header, HPackOptions()); }
        ...
        ssize_t Decode(flare::cord_buf *source, Header *h);

        // Like the previous function, except that the source is from
        // cord_buf_bytes_iterator.
        ssize_t Decode(flare::cord_buf_bytes_iterator &source, Header *h);
...
// Lowercase the input string, a fast implementation.
    ...
    inline ssize_t HPacker::Decode(flare::cord_buf *source, Header *h) {
        flare::cord_buf_bytes_iterator iter(*source);
