...
public:
    ...
    int CheckHealth(Socket* ptr) {
        if (ptr->health_check_count() == 0) {
            FLARE_LOG(INFO) << "Checking " << *chan << " chan=0x" << (void*)chan
                      << " Fake" << *ptr;
        }
    ...
    void AfterRevived(Socket* ptr) {
        FLARE_LOG(INFO) << "Revived " << *chan << " chan=0x" << (void*)chan
...
int ChannelBalancer::AddChannel(ChannelBase* sub_channel,
    ...
    if (nullptr == sub_channel) {
        FLARE_LOG(ERROR) << "Parameter[sub_channel] is nullptr";
        return -1;
    }
    FLARE_SCOPED_LOCK(_mutex);
    if (_chan_map.find(sub_channel) != _chan_map.end()) {
        FLARE_LOG(ERROR) << "Duplicated sub_channel=" << sub_channel;
        return -1;
    }
    SubChannel* sub_chan = new (std::nothrow) SubChannel;
    if (sub_chan == nullptr) {
        FLARE_LOG(FATAL) << "Fail to to new SubChannel";
        return -1;
    }
    ...
    if (Socket::Create(options, &sock_id) != 0) {
        delete sub_chan;
        FLARE_LOG(ERROR) << "Fail to create fake socket for sub channel";
        return -1;
    }
    SocketUniquePtr ptr;
    FLARE_CHECK_EQ(0, Socket::Address(sock_id, &ptr));
...
void ChannelBalancer::RemoveAndDestroyChannel(SelectiveChannel::ChannelHandle handle) {
    ...
    if (rc >= 0) {
        ...
        {
            FLARE_SCOPED_LOCK(_mutex);
            FLARE_CHECK_EQ(1UL, _chan_map.erase(sub->chan));
        }
        {
...
int Sender::IssueRPC(int64_t start_realtime_us) {
    ...
    if (rc != 0) {
        _main_cntl->SetFailed(rc, "Fail to select channel, {}", flare_error(rc));
        return -1;
    }
    FLARE_DLOG(INFO) << "Selected channel=" << sel_out.channel() << ", size="
...
void SubDone::Run() {
    ...
    if (rc != 0) {
        ...
        // all sub calls.
        FLARE_LOG(ERROR) << "Fail to lock correlation_id="
...
inline Resource Sender::PopFree() {
    if (_nfree == 0) {
        if (_nalloc == 0) {
            ...
            FLARE_CHECK(false) << "nalloc=" << _nalloc;
            return Resource();
        }
    } else {
...
int SelectiveChannel::Init(const char* lb_name, const ChannelOptions* options) {
    ...
    if (nullptr == lb) {
        FLARE_LOG(FATAL) << "Fail to new ChannelBalancer";
        return -1;
    }
    if (lb->Init(lb_name) != 0) {
        FLARE_LOG(ERROR) << "Fail to init lb";
        delete lb;
        return -1;
    }
    _chan._lb.reset(lb);
...
int SelectiveChannel::AddChannel(ChannelBase* sub_channel,
    ...
    if (lb == nullptr) {
        FLARE_LOG(ERROR) << "You must call Init() to initialize a SelectiveChannel";
        return -1;
    }
