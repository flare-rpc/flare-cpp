...
#include "flare/rpc/describable.h"               // Describable
#include "flare/rpc/circuit_breaker.h"           // CircuitBreaker
#include "flare/rpc/input_messenger.h"
#include "flare/rpc/details/sparse_minute_counter.h"
...
// Used by ConnectionService
    ...
    // must be even because Address() relies on evenness of version
            : _versioned_ref(0), _shared_part(NULL), _nevent(0), _keytable_pool(NULL), _fd(-1), _tos(0),
              ...
              _parsing_context(NULL), _correlation_id(0), _health_check_interval_s(-1), _ninprocess(1),
              _auth_flag_error(0), _auth_id(INVALID_FIBER_TOKEN), _auth_context(NULL), _ssl_state(SSL_UNKNOWN),
              _ssl_session(NULL), _connection_type_for_progressive_read(CONNECTION_TYPE_UNKNOWN),
              _controller_released_socket(false), _overcrowded(false), _fail_me_at_server_stop(false),
              _logoff_flag(false), _recycle_flag(false), _error_code(0), _pipeline_q(NULL), _last_writetime_us(0),
...
// `singular_node' is true iff `old_head' is the only node in its list.
    ...
    void *Socket::KeepWrite(void *void_arg) {
        ...
        do {
            ...
            if (nw <= 0) {
                ...
                // growing infinitely.
                const timespec duetime = flare::time_point::future_unix_millis(WAIT_EPOLLOUT_TIMEOUT_MS).to_timespec();
    ...
    int Socket::CheckHealth() {
        ...
        const timespec duetime = flare::time_point::future_unix_millis(FLAGS_health_check_timeout_ms).to_timespec();
...
namespace std {
    ostream &operator<<(ostream &os, const flare::rpc::Socket &sock) {
