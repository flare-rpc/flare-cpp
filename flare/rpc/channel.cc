...
#include "flare/rpc/details/usercode_backup_pool.h"       // TooManyUserCode
...
namespace flare::rpc {
    ...
    static ChannelSignature ComputeChannelSignature(const ChannelOptions &opt) {
        ...
        std::string buf;
        buf.reserve(1024);
        flare::hash::MurmurHash3_x64_128_Context mm_ctx;
        do {
            buf.clear();
            flare::hash::MurmurHash3_x64_128_Init(&mm_ctx, seed);
            ...
            flare::hash::MurmurHash3_x64_128_Update(&mm_ctx, buf.data(), buf.size());
            buf.clear();
            ...
            if (opt.has_ssl_options()) {
                ...
                if (!cert.certificate.empty()) {
                    ...
                    flare::hash::MurmurHash3_x64_128_Update(
            ...
            flare::hash::MurmurHash3_x64_128_Final(result.data, &mm_ctx);
    ...
    int Channel::InitSingle(const flare::base::end_point &server_addr_and_port,
        ...
        if (_options.protocol == flare::rpc::PROTOCOL_HTTP && scheme == "https://") {
    ...
    void Channel::CallMethod(const google::protobuf::MethodDescriptor *method,
        ...
        if (cntl->backup_request_ms() >= 0 &&
            ...
            const int rc = fiber_timer_add(
                    &cntl->_timeout_id,
                    flare::time_point::from_unix_micros(
