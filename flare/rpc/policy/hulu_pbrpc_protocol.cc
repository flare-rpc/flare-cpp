...
#include "flare/rpc/policy/hulu_pbrpc_meta.pb.h"      // HuluRpcRequestMeta
#include "flare/rpc/policy/hulu_pbrpc_protocol.h"
#include "flare/rpc/policy/most_common_message.h"
#include "flare/rpc/policy/hulu_pbrpc_controller.h"   // HuluController
...
static void SerializeHuluHeaderAndMeta(
    flare::cord_buf* out, const Meta& meta, int payload_size) {
...
ParseResult ParseHuluMessage(flare::cord_buf* source, Socket* socket,
...
void ProcessHuluRequest(InputMessageBase* msg_base) {
    ...
    do {
        ...
        flare::cord_buf* req_buf_ptr = &msg->payload;
        if (meta.has_user_message_size()) {
...
void ProcessHuluResponse(InputMessageBase* msg_base) {
    ...
    if (meta.error_code() != 0) {
        ...
        flare::cord_buf res_buf;
        flare::cord_buf* res_buf_ptr = &msg->payload;
        if (meta.has_user_message_size()) {
...
void PackHuluRequest(flare::cord_buf* req_buf,
                     SocketMessage**,
