...
#include "flare/rpc/http_status_code.h"             // HTTP_STATUS_*
...
namespace flare::rpc {
    ...
    namespace policy {
        ...
        DEFINE_string(http_header_of_user_ip, "", "http requests sent by proxies may "
                                                  ...
                                                  "flare will read ip:port from the specified header for "
        ...
        CommonStrings::CommonStrings()
                : ACCEPT("accept"), DEFAULT_ACCEPT("*/*"), USER_AGENT("user-agent"),
                  DEFAULT_USER_AGENT("flare/1.0 curl/7.0"), CONTENT_TYPE("content-type"),
        ...
        void ProcessHttpResponse(InputMessageBase *msg) {
            ...
            do {
                ...
                const int sc = res_header->status_code();
                if (sc < 200 || sc >= 300) {
                    std::string err = flare::string_format(
                            "HTTP/{}.{} {} {}",
                            res_header->major_version(),
                            res_header->minor_version(),
                            static_cast<int>(res_header->status_code()),
        ...
        void SerializeHttpRequest(flare::cord_buf * /*not used*/,
            ...
            if (cntl->has_log_id()) {
                hreq.SetHeader(common->LOG_ID,
                               flare::string_format("{}", (unsigned long long) cntl->log_id()));
            }
                    ...
                    */
                    ...
                    if (cntl->timeout_ms() >= 0) {
                        hreq.SetHeader(common->GRPC_TIMEOUT,
                                       flare::string_format("{} m", cntl->timeout_ms()));
                    }
            ...
            Span *span = accessor.span();
            if (span) {
                hreq.SetHeader("x-bd-trace-id", flare::string_format(
                        "{}", (unsigned long long) span->trace_id()));
                hreq.SetHeader("x-bd-span-id", flare::string_format(
                        "{}", (unsigned long long) span->span_id()));
                hreq.SetHeader("x-bd-parent-span-id", flare::string_format(
            ...
            if (cntl->Failed()) {
                ...
                if (!is_grpc) {
                    ...
                    res_header->SetHeader(common->ERROR_CODE,
                                          flare::string_format("{}", cntl->ErrorCode()));
