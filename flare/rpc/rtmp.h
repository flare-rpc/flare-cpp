...
public:
    AVCNaluIterator(flare::cord_buf* data, uint32_t length_size_minus1,
                    AVCNaluFormat* format_inout);
    ~AVCNaluIterator();
    void operator++();
    operator void*() const { return _data; }
    flare::cord_buf& operator*() { return _cur_nalu; }
    flare::cord_buf* operator->() { return &_cur_nalu; }
...
public:
    ...
    explicit FlvWriter(flare::cord_buf* buf);
