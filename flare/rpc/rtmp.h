...
#include "flare/rpc/controller.h"             // Controller, cord_buf
#include "flare/rpc/rtmp.pb.h"                // RtmpConnectRequest
#include "flare/rpc/amf.h"                    // AMFObject
...
struct AudioSpecificConfig {
    AudioSpecificConfig();
    flare::base::flare_status Create(const flare::io::cord_buf& buf);
...
public:
    AVCNaluIterator(flare::io::cord_buf* data, uint32_t length_size_minus1,
                    AVCNaluFormat* format_inout);
    ~AVCNaluIterator();
    void operator++();
    operator void*() const { return _data; }
    flare::io::cord_buf& operator*() { return _cur_nalu; }
    flare::io::cord_buf* operator->() { return &_cur_nalu; }
...
public:
    ...
    explicit FlvWriter(flare::io::cord_buf* buf);
    explicit FlvWriter(flare::io::cord_buf* buf, const FlvWriterOptions& options);
...
private:
    flare::base::flare_status WriteScriptData(const flare::io::cord_buf& req_buf, uint32_t timestamp);
...
public:
    ...
    explicit FlvReader(flare::io::cord_buf* buf);
...
public:
    ...
    virtual void OnCuePoint(flare::rpc::RtmpCuePoint* cuepoint) = 0;
    virtual void OnMetaData(flare::rpc::RtmpMetaData* metadata, const std::string_view& name) = 0;
    virtual void OnAudioMessage(flare::rpc::RtmpAudioMessage* msg) = 0;
    virtual void OnVideoMessage(flare::rpc::RtmpVideoMessage* msg) = 0;
...
public:
    ...
    void OnCuePoint(flare::rpc::RtmpCuePoint* cuepoint);
    void OnMetaData(flare::rpc::RtmpMetaData* metadata, const std::string_view& name);
