...
namespace flare::rpc {
    ...
    static bool find_avc_annexb_nalu_start_code(const flare::cord_buf &buf,
        ...
        for (flare::cord_buf_bytes_iterator it(buf); it != nullptr; ++it) {
            char c = *it;
            if (c == 0) {
...
// Shared by RtmpClient and RtmpClientStream(s)
    ...
    void RtmpConnect::StartConnect(
        ...
        if (ctx == nullptr) {
            FLARE_LOG(FATAL) << "RtmpContext of " << *s << " is nullptr";
...
// ============ RtmpClientStream =============
    ...
    void RtmpClientStream::OnStopInternal() {
        ...
        if (ctx != nullptr) {
            ...
            FLARE_LOG(FATAL) << "RtmpContext of " << *_rtmpsock << " is nullptr";
        }
        if (_from_socketmap) {
...
// ========= RtmpRetryingClientStream ============
    ...
    void RtmpRetryingClientStream::Init(
        ...
        if (sub_stream_creator == nullptr) {
            FLARE_LOG(ERROR) << "sub_stream_creator is nullptr";
            return CallOnStopIfNeeded();
        }
