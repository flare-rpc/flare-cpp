...
#ifndef FLARE_FUTURE_DETAIL_UTILITY_H_
#define FLARE_FUTURE_DETAIL_UTILITY_H_


#include <optional>
#include "flare/future/expected.h"
...
namespace flare::future_internal {
    ...
    struct get_first_error_impl<T, Ts...> {
        static std::optional<std::exception_ptr> exec(const flare::expected<T, std::exception_ptr> &first,
    ...
    auto fail_to_expect(const std::exception_ptr &src) {
        ...
        std::tuple<element_t> val_tup(flare::unexpected_type<std::exception_ptr>{src});
...
#endif  // FLARE_FUTURE_DETAIL_UTILITY_H_
