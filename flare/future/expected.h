...
#define ENABLE_IF_REQUIRES_T(...) \
    , typename = typename std::enable_if< (__VA_ARGS__), flare::expected_lite::detail::enabler >::type
...
namespace flare {
    namespace expected_lite {
        ...
        namespace detail {
            ...
            union storage_t {
                ...
                class flare::expected_lite::expected;
        ...
        public:
            ...
            template<typename E2 ENABLE_IF_REQUIRES_T(
                    ...
                    && !std::is_same<typename flare::remove_cvref<E2>::type, std::in_place_t>::value
                    && !std::is_same<typename flare::remove_cvref<E2>::type, unexpected_type>::value
            )
            >
    ...
    namespace expected_lite {
        ...
        public:
            using value_type = T;
            using error_type = E;
            using unexpected_type = flare::unexpected_type<E>;
            ...
            constexpr explicit expected(U &&value ENABLE_IF_REQUIRES_A(
                    std::is_constructible<T, U &&>::value
                    && !std::is_same<typename flare::remove_cvref<U>::type, std::in_place_t>::value
                    && !std::is_same<expected<T, E>, typename flare::remove_cvref<U>::type>::value
                    && !std::is_same<flare::unexpected_type<E>, typename flare::remove_cvref<U>::type>::value
            ...
            constexpr explicit expected(flare::unexpected_type<G> const &error ENABLE_IF_REQUIRES_A(
                    std::is_constructible<E, G const &>::value
                    && !std::is_convertible<G const &, E>::value /*=> explicit */ )
            )
                    : has_value_(false) {
            ...
            template<typename G = E>
            constexpr /*non-explicit*/ expected(flare::unexpected_type<G> const &error ENABLE_IF_REQUIRES_A(
                    std::is_constructible<E, G const &>::value
                    && std::is_convertible<G const &, E>::value /*=> non-explicit */ )
            )
                    : has_value_(false) {
            ...
            template<typename G = E>
            constexpr explicit expected(flare::unexpected_type<G> &&error ENABLE_IF_REQUIRES_A(
                    std::is_constructible<E, G &&>::value
            ...
            template<typename G = E>
            constexpr /*non-explicit*/ expected(flare::unexpected_type<G> &&error ENABLE_IF_REQUIRES_A(
                    std::is_constructible<E, G &&>::value
            ...
            >
            expected &operator=(flare::unexpected_type<G> const &error) {
            ...
            >
            expected &operator=(flare::unexpected_type<G> &&error) {
...
namespace std {
    ...
    template<typename T, typename E>
    struct hash<flare::expected<T, E> > {
        using result_type = std::size_t;
        using argument_type = flare::expected<T, E>;
    ...
    template<typename E>
    struct hash<flare::expected<void, E> > {
    };
...
#endif // FLARE_FUTURE_EXPECTED_H_