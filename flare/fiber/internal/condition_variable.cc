...
#include "flare/fiber/internal/types.h"                       // bthread_cond_t
...
namespace flare::fiber_internal {
    ...
    static_assert(sizeof(CondInternal) == sizeof(bthread_cond_t),
                  "sizeof_innercond_must_equal_cond");
    static_assert(offsetof(CondInternal, m) == offsetof(bthread_cond_t, m),
...
int bthread_cond_init(bthread_cond_t *__restrict c,
    ...
    c->m = NULL;
    c->seq = flare::fiber_internal::butex_create_checked<int>();
    *c->seq = 0;
    return 0;
}

int bthread_cond_destroy(bthread_cond_t *c) {
    flare::fiber_internal::butex_destroy(c->seq);
    c->seq = NULL;
    return 0;
}
...
int bthread_cond_signal(bthread_cond_t *c) {
    ...
    flare::fiber_internal::butex_wake(saved_seq);
    return 0;
}
...
int bthread_cond_broadcast(bthread_cond_t *c) {
    ...
    flare::fiber_internal::butex_requeue(saved_seq, saved_butex);
    return 0;
}
...
int bthread_cond_wait(bthread_cond_t *__restrict c,
    ...
    int rc1 = 0;
    if (flare::fiber_internal::butex_wait(ic->seq, expected_seq, NULL) < 0 &&
...
int bthread_cond_timedwait(bthread_cond_t *__restrict c,
    ...
    bthread_mutex_unlock(m);
    int rc1 = 0;
    if (flare::fiber_internal::butex_wait(ic->seq, expected_seq, abstime) < 0 &&
        errno != EWOULDBLOCK && errno != EINTR/*note*/) {
