        ...
        if (white_matcher.wildcards().empty() &&
            ...
            for (std::set<std::string>::const_iterator
                ...
                if (!black_matcher.match(name)) {
                    if (flare::variable::Variable::describe_exposed(
            ...
            flare::variable::Variable::list_exposed(&varnames, opt.display_filter);
...
// ============= export to files ==============
    ...
    public:
        ...
        bool dump(const std::string &name, const std::string_view &desc) override {
            if (_fp == NULL) {
                std::error_code ec;
                std::filesystem::path dirPath = std::filesystem::path(_filename).parent_path();
