...
#define TEMPLATE_OF_ADD_FUNC_BODY(fntype, pbtype)                       \
    ...
    "    return value.stream()->good();\n"                              \
    "  }\n"                                                             \
    "  FLARE_LOG(ERROR) << \"Can't set \" << value << \" to $field$ (repeated " #pbtype ")\";\n" \
    "  return false;\n"                                                 \
    "}\n"
...
static bool is_map_entry(const google::protobuf::Descriptor* d) {
    ...
    if (d->field(0)->is_repeated()) {
        FLARE_LOG(ERROR) << d->field(0)->full_name() << " must be required or optional";
        return false;
    }
    if (d->field(1)->is_repeated()) {
        FLARE_LOG(ERROR) << d->field(1)->full_name() << " must be required or optional";
        return false;
    }
    if (d->field(0)->cpp_type() != google::protobuf::FieldDescriptor::CPPTYPE_STRING) {
        FLARE_LOG(ERROR) << "key of idl map must be string";
        return false;
    }
    return true;
}
...
static bool generate_parsing(const google::protobuf::Descriptor* d,
    ...
    for (int i = 0; i < d->field_count(); ++i) {
        ...
        if (f->is_repeated()) {
            impl.Print("// repeated $type$ $name$ = $number$;\n"
                       ...
                       , "name", f->name()
                       , "number", flare::string_format("{}", f->number()));
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                impl.Print(
                    ...
                    "    return value.stream()->good();\n"
                    "  }\n"                                                             
                    "  FLARE_LOG(ERROR) << \"Can't set \" << value << \" to repeated $enum$\";\n"
                    "  return false;\n"                                                 
                    "}\n"
                    , "msg", cpp_name
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                impl.Print(
                    ...
                    "      } else {\n"
                    "        FLARE_LOG(ERROR) << \"Can't add \" << *it << \" to $field$ (repeated string)\";\n"
                    "        return false;\n"    
                    "      }\n"
                    "    }\n"
                    "    return value.stream()->good();\n"
                    "  }\n"                                                             
                    "  FLARE_LOG(ERROR) << \"Can't set \" << value << \" to $field$ (repeated string)\";\n"
                    "  return false;\n"    
                    "}\n"
                    , "msg", cpp_name
                    , "field", f->full_name()
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                ...
                if (is_map_entry(f->message_type())) {
                    ...
                    impl.Print(
                        ...
                        "    return value.stream()->good();\n"
                        "  }\n"
                        "  FLARE_LOG(ERROR) << \"Can't set \" << value << \" to $field$ (repeated $msg2$)\";\n"
                        "  return false;\n"    
                        "}\n"
                        , "vmsg2", var_name2
                ...
                impl.Print(
                    ...
                    "          continue;\n"
                    "        } else {\n"
                    "          FLARE_LOG(ERROR) << \"No field=\" << it->name << \" (\"\n"
                    "                     << value << \") in $msg$\";\n"
                    "          return false;\n"
                    "        }\n"
                    "      }\n"
                    ...
                    "            if (!(*fn)(sub_msg, *it2)) {\n"
                    "              FLARE_LOG(ERROR) << \"Fail to set item of \" << it->name;\n"
                    "              return false;\n"
                    "            }\n"
                    "          }\n"
                    "        }\n"
                    "      } else if (it->value.type() == ::mcpack2pb::FIELD_ISOARRAY) {\n"
                    "         FLARE_LOG(ERROR) << \"Shouldn't be a iso array, name=\" << it->name;\n"
                    "         return false;\n"
                    "      } else {\n"
                    "         FLARE_LOG(ERROR) << \"Can't add \" << value << \" to repeated $msg$\";\n"
                    "         return false;\n"
                    "      }\n"
                    "    }\n"
                    "    return value.stream()->good();\n"
                    ...
                    "        }\n"
                    "      } else {\n"
                    "        FLARE_LOG(ERROR) << \"Can't add \" << *it << \" to repeated $msg$\";\n"
                    "        return false;\n"    
                    "      }\n"
                    "    }\n"
                    "    return value.stream()->good();\n"
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                ...
                impl.Print(
                    ...
                    "    return value.stream()->good();\n"
                    "  }\n"
                    "  FLARE_LOG(ERROR) << \"Can't set \" << value << \" to $field$ (string)\";\n"
                    "  return false;\n"
                    "}\n"
                    , "msg", cpp_name
                    , "field", f->full_name()
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                ...
                impl.Print(
                    // FIXME: describe type.
                    "  FLARE_LOG(ERROR) << \"Can't set \" << value << \" to $field$\";\n"
                    "  return false;\n"
                    "}\n"
                    , "field", f->full_name());
            } break;
    ...
    impl.Print(
        ...
        "    FLARE_LOG(ERROR) << \"Missing required fields: \" << msg->InitializationErrorString();\n"
        "    return false;\n"
        "  }\n"
        "  return true;\n"
        "}\n"
...
#define TEMPLATE_SERIALIZE_REPEATED(cit, printer, field, strict_cond, looser_cond) \
    if (strict_cond) {                                                  \
        ...
        if ((field)->type() == google::protobuf::FieldDescriptor::TYPE_ENUM) { \
            FLARE_LOG(ERROR) << "Disallow converting " << (field)->full_name() \
...
static bool generate_serializing(const google::protobuf::Descriptor* d,
    ...
    for (int i = 0; i < d->field_count(); ++i) {
        ...
        if (cit == IDL_AUTO) {
            comment_template = flare::string_format(
                                "// {} $type$ $name$ = $number$;\n",
                                (f->is_repeated() ? "repeated" :
                                 (f->is_optional() ? "optional" : "required")));
        } else {
            comment_template = flare::string_format(
                                "// {} $type$ $name$ = $number$ [(idl_type)={}];\n",
                                (f->is_repeated() ? "repeated" :
                                 (f->is_optional() ? "optional" : "required")),
        ...
        if (f->is_repeated()) {
            ...
            case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                if (f->type() == google::protobuf::FieldDescriptor::TYPE_STRING) {
                    ...
                    FLARE_LOG(ERROR) << "Unknown pb type=" << f->type();
                    return false;
                }
                break;
            case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE: {
                if (cit != IDL_AUTO) {
                    FLARE_LOG(ERROR) << "Disallow converting " << f->full_name()
                ...
                for (int j = 0; j < msg2->field_count(); ++j) {
                    ...
                    if (f2->cpp_type() == google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE) {
                        if (cit2 != IDL_AUTO) {
                            FLARE_LOG(ERROR) << "Disallow converting " << f2->full_name()
                        ...
                        const std::string msgstr = flare::string_format(
                            "msg.{}(i)", f->lowercase_name().c_str());
                        switch (f2->cpp_type()) {
                        case google::protobuf::FieldDescriptor::CPPTYPE_INT32:
                        case google::protobuf::FieldDescriptor::CPPTYPE_UINT32:
                        case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
                        ...
                        case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
                            if (f2->type() == google::protobuf::FieldDescriptor::TYPE_STRING) {
                                ...
                                FLARE_LOG(ERROR) << "Unknown pb type=" << f2->type();
                                return false;
                            }
                            break;
                        case google::protobuf::FieldDescriptor::CPPTYPE_MESSAGE:
            ...
            if (f->is_required()) {
                impl.Print(" else {\n"
                           "  FLARE_LOG(ERROR) << \"Missing required $field$\";\n"
...
static std::string protobuf_style_normalize_filename(const std::string & fname) {
    ...
    for (size_t i = 0; i < fname.size(); ++i) {
        if (fname[i] == '_' || isdigit(fname[i]) || isalpha(fname[i])) {
            ...
            auto symbol = flare::string_format("_{:02x}", (int)fname[i]);
            norm_fname.append(symbol);
        }
    }
    return norm_fname;
}
...
static bool generate_registration(
    ...
    for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        impl.Print(
            ...
            "FLARE_CHECK_EQ(0, g_$vmsg$_fields->init(std::max($field_count$, 1), 30));\n"
            , "vmsg", var_name
            , "field_count", ::flare::string_format("{}", d->field_count()));
        for (int i = 0; i < d->field_count(); ++i) {
            const google::protobuf::FieldDescriptor* f = d->field(i);
...
bool McpackToProtobuf::Generate(const google::protobuf::FileDescriptor* file,
    ...
    if (pos == std::string::npos) {
        *error = ::flare::string_format("Bad filename={}", cpp_name.c_str());
        return false;
    }
    cpp_name.resize(pos);
    ...
    gdecl_printer.Print(
        "\n// ==== declarations generated by flare/mcpack2pb/protoc-gen-mcpack ====\n"
    ...
    for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        if (!generate_parsing(d, ref_msgs, ref_maps, gimpl_printer)) {
            *error = ::flare::string_format("Fail to generate parsing code for {}",
                d->full_name().c_str());
            return false;
        }
        if (!generate_serializing(d, ref_msgs, ref_maps, gimpl_printer)) {
            *error = ::flare::string_format("Fail to generate serializing code for {}",
                d->full_name().c_str());
            return false;
        }
        std::string var_name = mcpack2pb::to_var_name(d->full_name());
    ...
    if (!generate_declarations(ref_msgs, ref_maps, gdecl_printer)) {
        *error = ::flare::string_format("Fail to generate declarations for {}",
            cpp_name.c_str());
        return false;        
    }
    if (!generate_registration(file, gimpl_printer)) {
        *error = ::flare::string_format("Fail to generate registration code for {}",
            cpp_name.c_str());
        return false;
    }
    return true;
