...
void ObjectIterator::operator++() {
    if (_stream->popped_bytes() != _expected_popped_bytes) {
        if (_stream->popped_bytes() + _current_field.value.size() ==
            ...
            FLARE_CHECK(false) << "value of name=" << _current_field.name
                         << " is not fully consumed, expected="
                         << _expected_popped_bytes << " actually="
                         << _stream->popped_bytes();
            return set_bad();
        } else {
            FLARE_CHECK(false) << "Over popped in value of name=" << _current_field.name
    ...
    if (first_byte & FIELD_FIXED_MASK) {
        ...
        if (_stream->cut_packed_pod(&head) != sizeof(FieldFixedHead) ||
            ...
            FLARE_CHECK(false) << "buffer(size=" << left_size() << ") is not enough";
            return set_bad();
        }
...
void ArrayIterator::operator++() {
    if (_stream->popped_bytes() != _expected_popped_bytes) {
        if (_stream->popped_bytes() + _current_field.size() ==
            ...
            FLARE_CHECK(false) << "previous value is not fully consumed, expected="
                         << _expected_popped_bytes << " actually="
                         << _stream->popped_bytes();
            return set_bad();
        } else {
            FLARE_CHECK(false) << "Over popped in previous value, expected="
                       << _expected_popped_bytes << " actually="
                       << _stream->popped_bytes();
            return set_bad();
        }
...
size_t unbox(InputStream* stream) {
    ...
    if (stream->cut_packed_pod(&head) != sizeof(FieldLongHead)) {
        FLARE_CHECK(false) << "Input buffer is not enough";
        return 0;
    }
    if (head.type() != FIELD_OBJECT) {
        FLARE_CHECK(false) << "type=" << type2str(head.type()) << " is not object";
        return 0;
    }
    if (!(head.type() & FIELD_NON_DELETED_MASK)) {
        FLARE_CHECK(false) << "The item is deleted";
        return 0;
    }
    if (head.name_size() != 0) {
        FLARE_CHECK(false) << "The object should not have name";
        return 0;
    }
...
int64_t UnparsedValue::as_int64(const char* var) {
    ...
    case PRIMITIVE_FIELD_UINT64: {
        ...
        FLARE_CHECK(false) << "uint64=" << value << " to " << var << " overflows";
        _stream->set_bad();
    ...
    case PRIMITIVE_FIELD_FLOAT:
        FLARE_CHECK(false) << "Can't set float=" << _stream->cut_packed_pod<float>()
                     << " to " << var;
        _stream->set_bad();
        return 0;
    case PRIMITIVE_FIELD_DOUBLE:
        FLARE_CHECK(false) << "Can't set double=" << _stream->cut_packed_pod<double>()
                     << " to " << var;
        _stream->set_bad();
        return 0;
    }
    FLARE_CHECK(false) << "Can't set type=" << type2str(_type) << " to " << var;
    _stream->set_bad();
    return 0;
}
...
uint64_t UnparsedValue::as_uint64(const char* var) {
    ...
    case PRIMITIVE_FIELD_INT8: {
        ...
        FLARE_CHECK(false) << "Can't set int8=" << value << " to " << var;
        _stream->set_bad();
        return 0;
    }
    case PRIMITIVE_FIELD_INT16: {
        ...
        FLARE_CHECK(false) << "Can't set int16=" << value << " to " << var;
        _stream->set_bad();
        return 0;
    }
    case PRIMITIVE_FIELD_INT32: {
        ...
        FLARE_CHECK(false) << "Can't set int32=" << value << " to " << var;
        _stream->set_bad();
        return 0;
    }
    case PRIMITIVE_FIELD_INT64: {
        ...
        FLARE_CHECK(false) << "Can't set int64_t=" << value << " to " << var;
        _stream->set_bad();
        return 0;
    }
    case PRIMITIVE_FIELD_UINT8:
...
int32_t UnparsedValue::as_int32(const char* var) {
    ...
    case PRIMITIVE_FIELD_INT64: {
        ...
        if (value > std::numeric_limits<int32_t>::max()) {
            FLARE_CHECK(false) << "int64_t=" << value << " to " << var << " overflows";
            _stream->set_bad();
            return std::numeric_limits<int32_t>::max();
        } else if (value < std::numeric_limits<int32_t>::min()) {
            FLARE_CHECK(false) << "int64_t=" << value << " to " << var << " underflows";
            _stream->set_bad();
    ...
    case PRIMITIVE_FIELD_UINT32: {
        ...
        FLARE_CHECK(false) << "uint32=" << value << " to " << var << " overflows";
        _stream->set_bad();
        return std::numeric_limits<int32_t>::max();
    }
    case PRIMITIVE_FIELD_UINT64: {
...
void UnparsedValue::as_string(std::string* out, const char* var) {
    ...
    if (_stream->cutn(&(*out)[0], _size - 1) != _size - 1) {
        FLARE_CHECK(false) << "Not enough data for " << var;
        return;
    }
    _stream->popn(1);
}
