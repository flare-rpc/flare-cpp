...
namespace mcpack2pb {
    ...
    inline Serializer::GroupInfo *Serializer::push_group_info() {
        if (_ndepth + 1 < (int) FLARE_ARRAY_SIZE(_group_info_fast)) {
        ...
        if (_group_info_more == NULL) {
            _group_info_more =
                    (GroupInfo *) malloc((MAX_DEPTH + 1 - FLARE_ARRAY_SIZE(_group_info_fast))
                                         * sizeof(GroupInfo));
            if (_group_info_more == NULL) {
                return NULL;
            }
        }
        return &_group_info_more[++_ndepth - FLARE_ARRAY_SIZE(_group_info_fast)];
    }

    inline Serializer::GroupInfo &Serializer::peek_group_info() {
        if (_ndepth < (int) FLARE_ARRAY_SIZE(_group_info_fast)) {
            return _group_info_fast[_ndepth];
        } else {
            return _group_info_more[_ndepth - FLARE_ARRAY_SIZE(_group_info_fast)];
        }
