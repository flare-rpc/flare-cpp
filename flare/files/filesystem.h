...
#define GHC_PLATFORM_LITERAL(str) str
    ...
    void swap(file_path &lhs, file_path &rhs) noexcept;

    size_t hash_value(const file_path &p) noexcept;

#ifdef GHC_HAS_THREEWAY_COMP
    std::strong_ordering operator<=>(const file_path& lhs, const file_path& rhs) noexcept;
#endif

    bool operator==(const file_path &lhs, const file_path &rhs) noexcept;
...
// [fs.op.funcs] filesystem operations
    ...
    file_path absolute(const file_path &p);

    file_path canonical(const file_path &p);

    void copy(const file_path &from, const file_path &to);

    void copy(const file_path &from, const file_path &to, copy_options options);

    bool copy_file(const file_path &from, const file_path &to);

    bool copy_file(const file_path &from, const file_path &to, copy_options option);

    void copy_symlink(const file_path &existing_symlink, const file_path &new_symlink);

    bool create_directories(const file_path &p);

    bool create_directory(const file_path &p);

    bool create_directory(const file_path &p, const file_path &attributes);

    void create_directory_symlink(const file_path &to, const file_path &new_symlink);

    void create_symlink(const file_path &to, const file_path &new_symlink);

    file_path current_path();

    void current_path(const file_path &p);

    bool exists(const file_path &p);

    bool equivalent(const file_path &p1, const file_path &p2);

    uintmax_t file_size(const file_path &p);

    bool is_block_file(const file_path &p);

    bool is_character_file(const file_path &p);

    bool is_directory(const file_path &p);

    bool is_empty(const file_path &p);

    bool is_fifo(const file_path &p);

    bool is_other(const file_path &p);

    bool is_regular_file(const file_path &p);

    bool is_socket(const file_path &p);

    bool is_symlink(const file_path &p);

    file_time_type last_write_time(const file_path &p);

    void last_write_time(const file_path &p, file_time_type new_time);

    void permissions(const file_path &p, perms prms, perm_options opts = perm_options::replace);

    file_path proximate(const file_path &p, const file_path &base = current_path());

    file_path read_symlink(const file_path &p);

    file_path relative(const file_path &p, const file_path &base = current_path());

    bool remove(const file_path &p);

    uintmax_t remove_all(const file_path &p);

    void rename(const file_path &from, const file_path &to);

    void resize_file(const file_path &p, uintmax_t size);

    space_info space(const file_path &p);

    file_status status(const file_path &p);

    file_status symlink_status(const file_path &p);

    file_path temp_directory_path();

    file_path weakly_canonical(const file_path &p);

#endif

    file_path absolute(const file_path &p, std::error_code &ec);

    file_path canonical(const file_path &p, std::error_code &ec);

    void copy(const file_path &from, const file_path &to, std::error_code &ec) noexcept;

    void
    copy(const file_path &from, const file_path &to, copy_options options, std::error_code &ec) noexcept;

    bool copy_file(const file_path &from, const file_path &to, std::error_code &ec) noexcept;
    ...
    bool create_directories(const file_path &p, std::error_code &ec) noexcept;

    bool create_directory(const file_path &p, std::error_code &ec) noexcept;

    bool create_directory(const file_path &p, const file_path &attributes, std::error_code &ec) noexcept;

    /*
    template<typename Container, typename = typename std::enable_if<
    (flare::is_initializer_list<Container>::value ||
            (flare::has_value_type<Container>::value && flare::has_const_iterator<Container>::value))&&
            (std::is_same<std::string, typename Container::value_type>::value ||
            std::is_same<std::string_view, typename Container::value_type>::value ||
            std::is_same<file_path, typename Container::value_type>::value)>::type>
            */
    file_path join_path(const std::vector<std::string> &list);
    ...
    template<typename Container, typename = typename std::enable_if<
            ...
            (std::is_same<std::string, typename Container::value_type>::value ||
             ...
             std::is_same<file_path, typename Container::value_type>::value)>::type>
    */
    file_path join_path(const file_path &prefix, const std::vector<std::string> &list);

    void
    create_directory_symlink(const file_path &to, const file_path &new_symlink, std::error_code &ec) noexcept;

    void create_symlink(const file_path &to, const file_path &new_symlink, std::error_code &ec) noexcept;

    file_path current_path(std::error_code &ec);

    void current_path(const file_path &p, std::error_code &ec) noexcept;

    bool exists(file_status s) noexcept;

    bool exists(const file_path &p, std::error_code &ec) noexcept;

    bool equivalent(const file_path &p1, const file_path &p2, std::error_code &ec) noexcept;

    uintmax_t file_size(const file_path &p, std::error_code &ec) noexcept;

    bool is_block_file(file_status s) noexcept;

    bool is_block_file(const file_path &p, std::error_code &ec) noexcept;

    bool is_character_file(file_status s) noexcept;

    bool is_character_file(const file_path &p, std::error_code &ec) noexcept;

    bool is_directory(file_status s) noexcept;

    bool is_directory(const file_path &p, std::error_code &ec) noexcept;

    bool is_empty(const file_path &p, std::error_code &ec) noexcept;

    bool is_fifo(file_status s) noexcept;

    bool is_fifo(const file_path &p, std::error_code &ec) noexcept;

    bool is_other(file_status s) noexcept;

    bool is_other(const file_path &p, std::error_code &ec) noexcept;

    bool is_regular_file(file_status s) noexcept;

    bool is_regular_file(const file_path &p, std::error_code &ec) noexcept;

    bool is_socket(file_status s) noexcept;

    bool is_socket(const file_path &p, std::error_code &ec) noexcept;

    bool is_symlink(file_status s) noexcept;

    bool is_symlink(const file_path &p, std::error_code &ec) noexcept;

    file_time_type last_write_time(const file_path &p, std::error_code &ec) noexcept;

    void last_write_time(const file_path &p, file_time_type new_time, std::error_code &ec) noexcept;

    void permissions(const file_path &p, perms prms, std::error_code &ec) noexcept;

    void permissions(const file_path &p, perms prms, perm_options opts, std::error_code &ec) noexcept;

    file_path proximate(const file_path &p, std::error_code &ec);

    file_path proximate(const file_path &p, const file_path &base, std::error_code &ec);

    file_path read_symlink(const file_path &p, std::error_code &ec);

    file_path relative(const file_path &p, std::error_code &ec);

    file_path relative(const file_path &p, const file_path &base, std::error_code &ec);

    bool remove(const file_path &p, std::error_code &ec) noexcept;

    uintmax_t remove_all(const file_path &p, std::error_code &ec) noexcept;

    void rename(const file_path &from, const file_path &to, std::error_code &ec) noexcept;

    void resize_file(const file_path &p, uintmax_t size, std::error_code &ec) noexcept;

    space_info space(const file_path &p, std::error_code &ec) noexcept;

    file_status status(const file_path &p, std::error_code &ec) noexcept;

    bool status_known(file_status s) noexcept;

    file_status symlink_status(const file_path &p, std::error_code &ec) noexcept;

    file_path temp_directory_path(std::error_code &ec) noexcept;

    file_path weakly_canonical(const file_path &p, std::error_code &ec) noexcept;

#ifndef GHC_OS_WEB
#ifdef GHC_WITH_EXCEPTIONS

    void create_hard_link(const file_path &to, const file_path &new_hard_link);

    uintmax_t hard_link_count(const file_path &p);
    ...
    uintmax_t hard_link_count(const file_path &p, std::error_code &ec) noexcept;
...
inline std::wstring toWChar(const charT* unicodeString)
{
    ...
    inline void file_path::check_long_path() {
    ...
    inline void file_path::postprocess_path_with_format(file_path::format fmt) {
#ifdef GHC_RAISE_UNICODE_ERRORS
...
inline void create_symlink(const file_path& target_name, const file_path& new_symlink, bool to_directory, std::error_code& ec)
{
    std::error_code tec;
...
inline void create_hardlink(const file_path& target_name, const file_path& new_hardlink, std::error_code& ec)
{
#if defined(__GNUC__) && __GNUC__ >= 8
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wcast-function-type"
...
inline file_path getFullPathName(const wchar_t* p, std::error_code& ec)
{
...
inline std::unique_ptr<REPARSE_DATA_BUFFER, free_deleter<REPARSE_DATA_BUFFER>> getReparseData(const file_path& p, std::error_code& ec)
{
    ...
    return nullptr;
        ...
        inline file_path resolveSymlink(const file_path &p, std::error_code &ec) {
#ifdef GHC_OS_WINDOWS
                                                                                                                                    file_path result;
...
template <typename INFO>
inline file_status status_from_INFO(const file_path& p, const INFO* info, std::error_code& ec, uintmax_t* sz = nullptr, time_t* lwt = nullptr)
{
    file_type ft = file_type::unknown;
...
// [fs.file_path.construct] constructors and destructor
    ...
    inline file_path::file_path() noexcept {}

    inline file_path::file_path(const file_path &p)
            : _path(p._path)
#if defined(GHC_OS_WINDOWS) && defined(GHC_WIN_AUTO_PREFIX_LONG_PATH)
    ...
    inline file_path::file_path(file_path &&p) noexcept
    ...
    inline file_path::file_path(string_type &&source, format fmt)
    ...
    inline file_path::~file_path() {}

//-----------------------------------------------------------------------------
// [fs.file_path.assign] assignments

    inline file_path &file_path::operator=(const file_path &p) {
        _path = p._path;
#if defined(GHC_OS_WINDOWS) && defined(GHC_WIN_AUTO_PREFIX_LONG_PATH)
        _prefixLength = p._prefixLength;
#endif
    ...
    inline file_path &file_path::operator=(file_path &&p) noexcept {
        _path = std::move(p._path);
#if defined(GHC_OS_WINDOWS) && defined(GHC_WIN_AUTO_PREFIX_LONG_PATH)
        _prefixLength = p._prefixLength;
#endif
    ...
    inline file_path &file_path::operator=(file_path::string_type &&source) {
        return assign(source);
    }

    inline file_path &file_path::assign(file_path::string_type &&source) {
        _path = std::move(source);
        postprocess_path_with_format(native_format);
        return *this;
    }
...
// [fs.file_path.append] appends
    ...
    inline void file_path::append_name(const value_type *name) {
        if (_path.empty()) {
...
// [fs.file_path.concat] concatenation
    ...
    inline file_path &file_path::operator+=(const file_path &x) {
        return concat(x._path);
    }

    inline file_path &file_path::operator+=(const string_type &x) {
        return concat(x);
    }

    inline file_path &file_path::operator+=(basic_string_view<value_type> x) {
        return concat(x);
    }


    inline file_path &file_path::operator+=(const value_type *x) {
        basic_string_view<value_type> part(x);
        return concat(part);
    }

    inline file_path &file_path::operator+=(value_type x) {
#ifdef GHC_OS_WINDOWS
...
//-----------------------------------------------------------------------------
// [fs.file_path.modifiers] modifiers
    inline void file_path::clear() noexcept {
        _path.clear();
#if defined(GHC_OS_WINDOWS) && defined(GHC_WIN_AUTO_PREFIX_LONG_PATH)
        _prefixLength = 0;
#endif
    ...
    inline file_path &file_path::replace_filename(const file_path &replacement) {
        remove_filename();
        return append(replacement);
    }

    inline file_path &file_path::replace_extension(const file_path &replacement) {
        if (has_extension()) {
    ...
    inline void file_path::swap(file_path &rhs) noexcept {
...
//-----------------------------------------------------------------------------
// [fs.file_path.native.obs] native format observers
    inline const file_path::string_type &file_path::native() const noexcept {
        return _path;
    }

    inline const file_path::value_type *file_path::c_str() const noexcept {
        return native().c_str();
    }

    inline file_path::operator file_path::string_type() const {
        return native();
    }
    ...
    inline std::string file_path::string() const {
#ifdef GHC_USE_WCHAR_T
    ...
    inline std::wstring file_path::wstring() const {
#ifdef GHC_USE_WCHAR_T
        return native();
#else
...
#if defined(__cpp_lib_char8_t) && !defined(GHC_FILESYSTEM_ENFORCE_CPP17_API)
                                                                                                                            inline std::u8string file_path::u8string() const
{
#ifdef GHC_USE_WCHAR_T
    ...
    inline std::string file_path::u8string() const {
#ifdef GHC_USE_WCHAR_T
        return detail::toUtf8(native());
#else
        return native();
    ...
    inline std::u16string file_path::u16string() const {
        // TODO: optimize
        return detail::fromUtf8<std::u16string>(string());
    }

    inline std::u32string file_path::u32string() const {
        // TODO: optimize
...
// [fs.file_path.generic.obs] generic format observers
    ...
    inline std::string file_path::generic_string() const {
#ifdef GHC_OS_WINDOWS
    ...
    inline std::wstring file_path::generic_wstring() const {
#ifdef GHC_OS_WINDOWS
...
#if defined(__cpp_lib_char8_t) && !defined(GHC_FILESYSTEM_ENFORCE_CPP17_API)
                                                                                                                            inline std::u8string file_path::generic_u8string() const
{
#ifdef GHC_OS_WINDOWS
    ...
    inline std::string file_path::generic_u8string() const {
#ifdef GHC_OS_WINDOWS
        return generic_string<std::string::value_type, std::string::traits_type, std::string::allocator_type>();
#else
        return _path;
    ...
    inline std::u16string file_path::generic_u16string() const {
#ifdef GHC_OS_WINDOWS
    ...
    inline std::u32string file_path::generic_u32string() const {
#ifdef GHC_OS_WINDOWS
...
//-----------------------------------------------------------------------------
// [fs.file_path.compare] compare
    inline int file_path::compare(const file_path &p) const noexcept {
    ...
    inline int file_path::compare(const string_type &s) const {
        return compare(file_path(s));
    }

    inline int file_path::compare(basic_string_view<value_type> s) const {
        return compare(file_path(s));
    }


    inline int file_path::compare(const value_type *s) const {
        return compare(file_path(s));
    }
...
// [fs.file_path.decompose] decomposition
#ifdef GHC_OS_WINDOWS
                                                                                                                            inline void file_path::handle_prefixes()
{
    ...
    inline file_path::string_type::size_type file_path::root_name_length() const noexcept {
#ifdef GHC_OS_WINDOWS
    ...
    inline file_path file_path::root_name() const {
        return file_path(_path.substr(_prefixLength, root_name_length()), native_format);
    }

    inline file_path file_path::root_directory() const {
    ...
    inline file_path file_path::root_path() const {
        return file_path(root_name().string() + root_directory().string(), native_format);
    }

    inline file_path file_path::relative_path() const {
    ...
    inline file_path file_path::parent_path() const {
        auto rootPathLen = _prefixLength + root_name_length() + (has_root_directory() ? 1 : 0);
    ...
    inline file_path file_path::filename() const {
        return !has_relative_path() ? file_path() : file_path(*--end());
    }

    inline file_path file_path::stem() const {
    ...
    inline file_path file_path::extension() const {
...
#ifdef GHC_OS_WINDOWS
                                                                                                                            namespace detail {
inline bool has_executable_extension(const file_path& p)
{
...
//-----------------------------------------------------------------------------
// [fs.file_path.query] query
    inline bool file_path::empty() const noexcept {
        return _path.empty();
    }

    inline bool file_path::has_root_name() const {
        return root_name_length() > 0;
    }

    inline bool file_path::has_root_directory() const {
        auto rootLen = _prefixLength + root_name_length();
        return (_path.length() > rootLen && _path[rootLen] == preferred_separator);
    }

    inline bool file_path::has_root_path() const {
        return has_root_name() || has_root_directory();
    }

    inline bool file_path::has_relative_path() const {
        auto rootPathLen = _prefixLength + root_name_length() + (has_root_directory() ? 1 : 0);
        return rootPathLen < _path.length();
    }

    inline bool file_path::has_parent_path() const {
        return !parent_path().empty();
    }

    inline bool file_path::has_filename() const {
        return has_relative_path() && !filename().empty();
    }

    inline bool file_path::has_stem() const {
        return !stem().empty();
    }

    inline bool file_path::has_extension() const {
        return !extension().empty();
    }

    inline bool file_path::is_absolute() const {
#ifdef GHC_OS_WINDOWS
        return has_root_name() && has_root_directory();
#else
    ...
    inline bool file_path::is_relative() const {
        return !is_absolute();
    }

//-----------------------------------------------------------------------------
// [fs.file_path.gen] generation
    inline file_path file_path::lexically_normal() const {
        file_path dest;
    ...
    inline file_path file_path::lexically_relative(const file_path &base) const {
    ...
    inline file_path file_path::lexically_proximate(const file_path &base) const {
...
//-----------------------------------------------------------------------------
// [fs.file_path.itr] iterators
    inline file_path::iterator::iterator() {}

    inline file_path::iterator::iterator(const file_path &p, const impl_string_type::const_iterator &pos)
    ...
    inline file_path::impl_string_type::const_iterator
    ...
    inline void file_path::iterator::updateCurrent() {
        if ((_iter == _last) ||
    ...
    inline file_path::iterator &file_path::iterator::operator++() {
    ...
    inline file_path::iterator file_path::iterator::operator++(int) {
    ...
    inline bool file_path::iterator::operator==(const file_path::iterator &other) const {
    ...
    inline file_path::iterator::reference file_path::iterator::operator*() const {
        return _current;
    }

    inline file_path::iterator::pointer file_path::iterator::operator->() const {
        return &_current;
    }

    inline file_path::iterator file_path::begin() const {
        return iterator(*this, _path.begin());
    }

    inline file_path::iterator file_path::end() const {
        return iterator(*this, _path.end());
    }

//-----------------------------------------------------------------------------
// [fs.file_path.nonmember] file_path non-member functions
    inline void swap(file_path &lhs, file_path &rhs) noexcept {
        swap(lhs._path, rhs._path);
    }

    inline size_t hash_value(const file_path &p) noexcept {
        return std::hash<std::string>()(p.generic_string());
    }

#ifdef GHC_HAS_THREEWAY_COMP
                                                                                                                            inline std::strong_ordering operator<=>(const file_path& lhs, const file_path& rhs) noexcept
{
    ...
    inline file_path operator/(const file_path &lhs, const file_path &rhs) {
...
// [fs.class.filesystem_error] Class filesystem_error
    ...
    inline filesystem_error::filesystem_error(const std::string &what_arg, const file_path &p1,
                                              std::error_code ec)
    ...
    inline const file_path &filesystem_error::path1() const noexcept {
        return _p1;
    }

    inline const file_path &filesystem_error::path2() const noexcept {
        return _p2;
    }
...
// [fs.op.funcs] filesystem operations
    ...
    inline file_path absolute(const file_path &p) {
        std::error_code ec;
    ...
    inline file_path absolute(const file_path &p, std::error_code &ec) {
        ec.clear();
#ifdef GHC_OS_WINDOWS
                                                                                                                                if (p.empty()) {
    ...
    inline file_path canonical(const file_path &p) {
        std::error_code ec;
    ...
    inline file_path canonical(const file_path &p, std::error_code &ec) {
        if (p.empty()) {
    ...
    inline void copy(const file_path &from, const file_path &to) {
        copy(from, to, copy_options::none);
    }

    inline void copy(const file_path &from, const file_path &to, copy_options options) {
        std::error_code ec;
    ...
    inline void copy(const file_path &from, const file_path &to, std::error_code &ec) noexcept {
    ...
    inline file_path join_path(const std::vector<std::string> &list) {
        file_path p;
        return join_path(p, list);
    }

    inline file_path join_path(const file_path &prefix, const std::vector<std::string> &list) {
        file_path p(prefix);
        for(auto &c : list) {
            p /=c;
        }
    ...
    inline bool copy_file(const file_path &from, const file_path &to) {
        return copy_file(from, to, copy_options::none);
    }

    inline bool copy_file(const file_path &from, const file_path &to, copy_options option) {
        std::error_code ec;
    ...
    inline bool copy_file(const file_path &from, const file_path &to, std::error_code &ec) noexcept {
    ...
    inline void copy_symlink(const file_path &existing_symlink, const file_path &new_symlink) {
        std::error_code ec;
    ...
    inline bool create_directories(const file_path &p) {
        std::error_code ec;
    ...
    inline bool create_directories(const file_path &p, std::error_code &ec) noexcept {
        file_path current;
        ec.clear();
    ...
    inline bool create_directory(const file_path &p) {
        std::error_code ec;
    ...
    inline bool create_directory(const file_path &p, std::error_code &ec) noexcept {
        return create_directory(p, file_path(), ec);
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline bool create_directory(const file_path &p, const file_path &attributes) {
        std::error_code ec;
    ...
    inline void create_directory_symlink(const file_path &to, const file_path &new_symlink) {
        std::error_code ec;
        create_directory_symlink(to, new_symlink, ec);
        if (ec) {
    ...
    inline void create_hard_link(const file_path &to, const file_path &new_hard_link) {
        std::error_code ec;
    ...
    inline void create_symlink(const file_path &to, const file_path &new_symlink) {
        std::error_code ec;
        create_symlink(to, new_symlink, ec);
        if (ec) {
            throw filesystem_error(detail::systemErrorText(ec.value()), to, new_symlink, ec);
    ...
    inline file_path current_path(std::error_code &ec) {
        ec.clear();
#ifdef GHC_OS_WINDOWS
    ...
    inline void current_path(const file_path &p) {
        std::error_code ec;
        current_path(p, ec);
        if (ec) {
            throw filesystem_error(detail::systemErrorText(ec.value()), p, ec);
    ...
    inline void current_path(const file_path &p, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
    ...
    inline bool exists(const file_path &p) {
        return exists(status(p));
    }

#endif

    inline bool exists(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool equivalent(const file_path &p1, const file_path &p2) {
        std::error_code ec;
    ...
    inline bool equivalent(const file_path &p1, const file_path &p2, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
    ...
    inline uintmax_t file_size(const file_path &p) {
        std::error_code ec;
    ...
    inline uintmax_t file_size(const file_path &p, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
                                                                                                                                WIN32_FILE_ATTRIBUTE_DATA attr;
    if (!GetFileAttributesExW(GHC_NATIVEWP(p), GetFileExInfoStandard, &attr)) {
    ...
    inline uintmax_t hard_link_count(const file_path &p) {
        std::error_code ec;
    ...
    inline uintmax_t hard_link_count(const file_path &p, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
    ...
    inline bool is_block_file(const file_path &p) {
        return is_block_file(status(p));
    }

#endif

    inline bool is_block_file(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool is_character_file(const file_path &p) {
        return is_character_file(status(p));
    }

#endif

    inline bool is_character_file(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool is_directory(const file_path &p) {
        return is_directory(status(p));
    }

#endif

    inline bool is_directory(const file_path &p, std::error_code &ec) noexcept {
        return is_directory(status(p, ec));
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline bool is_empty(const file_path &p) {
        if (is_directory(p)) {
    ...
    inline bool is_empty(const file_path &p, std::error_code &ec) noexcept {
        auto fs = status(p, ec);
        if (ec) {
            return false;
        }
    ...
    inline bool is_fifo(const file_path &p) {
        return is_fifo(status(p));
    }

#endif

    inline bool is_fifo(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool is_other(const file_path &p) {
        return is_other(status(p));
    }

#endif

    inline bool is_other(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool is_regular_file(const file_path &p) {
        return is_regular_file(status(p));
    }

#endif

    inline bool is_regular_file(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool is_socket(const file_path &p) {
        return is_socket(status(p));
    }

#endif

    inline bool is_socket(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline bool is_symlink(const file_path &p) {
        return is_symlink(symlink_status(p));
    }

#endif

    inline bool is_symlink(const file_path &p, std::error_code &ec) noexcept {
        return is_symlink(symlink_status(p, ec));
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline file_time_type last_write_time(const file_path &p) {
        std::error_code ec;
    ...
    inline file_time_type last_write_time(const file_path &p, std::error_code &ec) noexcept {
        time_t result = 0;
        ec.clear();
    ...
    inline void last_write_time(const file_path &p, file_time_type new_time) {
        std::error_code ec;
    ...
    inline void last_write_time(const file_path &p, file_time_type new_time, std::error_code &ec) noexcept {
        ec.clear();
...
#define UTIME_OMIT ((1l << 30) - 2l)
    ...
    inline void permissions(const file_path &p, perms prms, perm_options opts) {
        std::error_code ec;
    ...
    inline void permissions(const file_path &p, perms prms, std::error_code &ec) noexcept {
        permissions(p, prms, perm_options::replace, ec);
    }

    inline void permissions(const file_path &p, perms prms, perm_options opts, std::error_code &ec) noexcept {
    ...
    inline file_path proximate(const file_path &p, std::error_code &ec) {
    ...
    inline file_path proximate(const file_path &p, const file_path &base) {
        return weakly_canonical(p).lexically_proximate(weakly_canonical(base));
    }

#endif

    inline file_path proximate(const file_path &p, const file_path &base, std::error_code &ec) {
        return weakly_canonical(p, ec).lexically_proximate(weakly_canonical(base, ec));
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline file_path read_symlink(const file_path &p) {
        std::error_code ec;
    ...
    inline file_path read_symlink(const file_path &p, std::error_code &ec) {
    ...
    inline file_path relative(const file_path &p, std::error_code &ec) {
        return relative(p, current_path(ec), ec);
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline file_path relative(const file_path &p, const file_path &base) {
        return weakly_canonical(p).lexically_relative(weakly_canonical(base));
    }

#endif

    inline file_path relative(const file_path &p, const file_path &base, std::error_code &ec) {
        return weakly_canonical(p, ec).lexically_relative(weakly_canonical(base, ec));
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline bool remove(const file_path &p) {
        std::error_code ec;
    ...
    inline bool remove(const file_path &p, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
                                                                                                                                #ifdef GHC_USE_WCHAR_T
    auto cstr = p.c_str();
    ...
    inline void rename(const file_path &from, const file_path &to) {
        std::error_code ec;
        rename(from, to, ec);
        if (ec) {
            throw filesystem_error(detail::systemErrorText(ec.value()), from, to, ec);
    ...
    inline void rename(const file_path &from, const file_path &to, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
                                                                                                                                if (from != to) {
    ...
    inline void resize_file(const file_path &p, uintmax_t size) {
        std::error_code ec;
    ...
    inline void resize_file(const file_path &p, uintmax_t size, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
    ...
    inline space_info space(const file_path &p) {
        std::error_code ec;
    ...
    inline space_info space(const file_path &p, std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
    ...
    inline file_status status(const file_path &p) {
        std::error_code ec;
    ...
    inline file_status status(const file_path &p, std::error_code &ec) noexcept {
    ...
    inline file_status symlink_status(const file_path &p) {
        std::error_code ec;
    ...
    inline file_status symlink_status(const file_path &p, std::error_code &ec) noexcept {
        return detail::symlink_status_ex(p, ec);
    }

#ifdef GHC_WITH_EXCEPTIONS

    inline file_path temp_directory_path() {
        std::error_code ec;
    ...
    inline file_path temp_directory_path(std::error_code &ec) noexcept {
        ec.clear();
#ifdef GHC_OS_WINDOWS
                                                                                                                                wchar_t buffer[512];
    ...
    inline file_path weakly_canonical(const file_path &p) {
        std::error_code ec;
    ...
    inline file_path weakly_canonical(const file_path &p, std::error_code &ec) noexcept {
        file_path result;
        ec.clear();
        bool scan = true;
        for (auto pe : p) {
...
// directory_entry::directory_entry(directory_entry&&) noexcept = default;
    ...
    inline directory_entry::directory_entry(const flare::file_path &p)
    ...
    inline directory_entry::directory_entry(const flare::file_path &p, std::error_code &ec)
            : _path(p), _file_size(static_cast<uintmax_t>(-1))
#ifndef GHC_OS_WINDOWS
            , _hard_link_count(static_cast<uintmax_t>(-1))
#endif
...
// [fs.dir.entry.mods] directory_entry modifiers
    ...
    inline void directory_entry::assign(const flare::file_path &p) {
        _path = p;
        refresh();
    }
    ...
    inline void directory_entry::assign(const flare::file_path &p, std::error_code &ec) {
        _path = p;
        refresh(ec);
    }
    ...
    inline void directory_entry::replace_filename(const flare::file_path &p) {
    ...
    inline void directory_entry::replace_filename(const flare::file_path &p, std::error_code &ec) {
        _path.replace_filename(p);
        refresh(ec);
    }
...
// [fs.dir.entry.obs] directory_entry observers
    inline const flare::file_path &directory_entry::file_path() const noexcept {
        return _path;
    }

    inline directory_entry::operator const flare::file_path &() const noexcept {
        return _path;
    }
...
// [fs.dir.itr.members] member functions
    ...
    inline directory_iterator::directory_iterator(const file_path &p)
    ...
    inline directory_iterator::directory_iterator(const file_path &p, directory_options options)
    ...
    inline directory_iterator::directory_iterator(const file_path &p, std::error_code &ec) noexcept
            : _impl(new impl(p, directory_options::none)) {
        if (_impl->_ec) {
            ec = _impl->_ec;
        }
...
// [fs.class.rec.dir.itr] class recursive_directory_iterator
    ...
    inline recursive_directory_iterator::recursive_directory_iterator(const file_path &p)
            : _impl(new recursive_directory_iterator_impl(directory_options::none, true)) {
