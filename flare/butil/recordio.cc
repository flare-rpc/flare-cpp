...
// name="CRC-8/KOOP"
    ...
    bool Record::RemoveMeta(const std::string_view &name) {
        for (size_t i = 0; i < _metas.size(); ++i) {
            if (_metas[i].name == name) {
    ...
    int RecordReader::CutRecord(Record *rec) {
        ...
        if (cut_bytes != after_payload_size) {
            LOG(ERROR) << "cut_bytes=" << cut_bytes << " does not match after_payload_size=" << after_payload_size
                       << " previous_size=" << previous_payload_size << " data_size=" << data_size << " consumed_bytes="
                       << consumed_bytes;
        }
