...
// g_native_tls_key is the one native TLS that we use.  It stores our table.
    std::atomic<int64_t> g_native_tls_key =
...
// require memory allocations.
    void **ConstructTlsVector() {
        ...
        if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
            ...
            int64_t old =PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES;
            auto r = g_native_tls_key.compare_exchange_weak(old, key);
    ...
    void OnThreadExitInternal(void *value) {
        ...
        while (need_to_scan_destructors) {
            ...
            int32_t last_used_tls_key = g_last_used_tls_key.load();
...
namespace butil {
    ...
    bool ThreadLocalStorage::StaticSlot::Initialize(TLSDestructorFunc destructor) {
        ...
        // Grab a new slot.
        slot_ = g_last_used_tls_key.fetch_add( 1)+1;
        DCHECK_GT(slot_, 0);
    ...
    void *ThreadLocalStorage::StaticSlot::Get() const {
        void **tls_data = static_cast<void **>(
                PlatformThreadLocalStorage::GetTLSValue(g_native_tls_key.load()));
        if (!tls_data)
