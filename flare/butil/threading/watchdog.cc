...
// Notify watchdog thread, and wait for it to finish up.
    ...
    void Watchdog::Cleanup() {
        if (!enabled_)
            return;
        {
            flare::base::AutoLock lock(lock_);
            state_ = SHUTDOWN;
        }
        condition_variable_.Signal();
    }
...
// Internal private methods that the watchdog thread uses.
    ...
    void Watchdog::ThreadDelegate::ThreadMain() {
        ...
        while (1) {
            flare::base::AutoLock lock(watchdog_->lock_);
            ...
            {
                flare::base::AutoLock static_lock(static_data->lock);
            ...
            {
                flare::base::AutoUnlock lock(watchdog_->lock_);
...
// static
    void Watchdog::ResetStaticData() {
        StaticData *static_data = g_static_data.Pointer();
        flare::base::AutoLock lock(static_data->lock);
