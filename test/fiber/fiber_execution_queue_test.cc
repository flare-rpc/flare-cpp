...
struct LongIntTask {
    ...
    LongIntTask(long v)
        : value(v), event(NULL)
    {}
    LongIntTask(long v, flare::fiber_internal::CountdownEvent* e)
        : value(v), event(e)
    {}
...
void* push_thread(void *arg) {
    ...
    timer.start();
    int num = 0;
    flare::fiber_internal::CountdownEvent e;
...
TEST_F(ExecutionQueueTest, urgent_task_is_the_last_task) {
    ...
    ASSERT_EQ(0, flare::fiber_internal::execution_queue_execute(queue_id, -1, &flare::fiber_internal::TASK_OPTIONS_URGENT));
    usleep(100);
    g_suspending = false;
