...
TEST_F(StringSplitterTest, sanity) {
    ...
    flare::base::StringSplitter ss(str, ' ');
    // "hello"
    ASSERT_TRUE(ss != NULL);
    ...
    flare::base::StringSplitter ss2(str, ' ', flare::base::ALLOW_EMPTY_FIELD);
...
TEST_F(StringSplitterTest, number_list) {
    ...
    flare::base::StringMultiSplitter ss(str, ", ");
    ASSERT_TRUE(ss);
    ASSERT_EQ(3ul, ss.length());
    ASSERT_FALSE(strncmp(ss.field(), "123", ss.length()));
    ...
    flare::base::StringMultiSplitter ss2(str, str + str_len, ", ");
    ASSERT_TRUE(ss2);
    ASSERT_EQ(3ul, ss2.length());
...
TEST_F(StringSplitterTest, split_limit_len) {
    ...
    flare::base::StringSplitter ss(str, str + 5, '\t');
    ...
    flare::base::StringSplitter ss2(str, str + 5, '\0');
    ...
    // Allows using '\0' as separator
    flare::base::StringSplitter ss3(sp, '\0');
...
TEST_F(StringSplitterTest, key_value_pairs_splitter_sanity) {
    ...
    for (int i = 0 ; i < 3; ++i) {
        ...
        flare::base::KeyValuePairsSplitter* psplitter = NULL;
        if (i == 0) {
            psplitter = new flare::base::KeyValuePairsSplitter(kvstr, '&', '=');
        } else if (i == 1) {
            psplitter = new flare::base::KeyValuePairsSplitter(
                    kvstr.data(), kvstr.data() + kvstr.size(), '&', '=');
        } else if (i == 2) {
            psplitter = new flare::base::KeyValuePairsSplitter(kvstr.c_str(), '&', '=');
        }
        flare::base::KeyValuePairsSplitter& splitter = *psplitter;
