...
namespace butil {
    ...
    TEST(SmallMap, General) {
        SmallMap<flare::container::hash_map<int, int> > m;
        ...
        SmallMap<flare::container::hash_map<int, int> >::iterator iter(m.begin());
        ...
        const SmallMap<flare::container::hash_map<int, int> > &ref = m;
    ...
    TEST(SmallMap, PostFixIteratorIncrement) {
        ...
        {
            SmallMap<flare::container::hash_map<int, int> >::iterator iter(m.begin());
            SmallMap<flare::container::hash_map<int, int> >::iterator last(iter++);
            ++last;
            EXPECT_TRUE(last == iter);
        }

        {
            SmallMap<flare::container::hash_map<int, int> >::const_iterator iter(m.begin());
            SmallMap<flare::container::hash_map<int, int> >::const_iterator last(iter++);
            ++last;
            EXPECT_TRUE(last == iter);
        }
    }
...
// Based on the General testcase.
    TEST(SmallMap, CopyConstructor) {
        SmallMap<flare::container::hash_map<int, int> > src;

        {
            SmallMap<flare::container::hash_map<int, int> > m(src);
            EXPECT_TRUE(m.empty());
        }
    ...
    TEST(SmallMap, AssignmentOperator) {
        SmallMap<flare::container::hash_map<int, int> > src_small;
        SmallMap<flare::container::hash_map<int, int> > src_large;
        ...
        SmallMap<flare::container::hash_map<int, int> > dest_small;
        ...
        SmallMap<flare::container::hash_map<int, int> > dest_large;
    ...
    TEST(SmallMap, Insert) {
        SmallMap<flare::container::hash_map<int, int> > sm;

        // loop through the transition from small map to map.
        for (int i = 1; i <= 10; ++i) {
            VLOG(1) << "Iteration " << i;
            // insert an element
            std::pair<SmallMap<flare::container::hash_map<int, int> >::iterator,
                    bool> ret;
            ...
            for (int j = 1; j <= i; ++j) {
                SmallMap<flare::container::hash_map<int, int> >::iterator it = sm.find(j);
    ...
    TEST(SmallMap, InsertRange) {
        ...
        for (int elements = 0; elements <= 10; ++elements) {
            ...
            flare::container::hash_map<int, int> normal_map;
    ...
    TEST(SmallMap, Erase) {
        SmallMap<flare::container::hash_map<std::string, int> > m;
        SmallMap<flare::container::hash_map<std::string, int> >::iterator iter;
    ...
    TEST(SmallMap, DefaultEqualKeyWorks) {
        ...
        SmallMap<flare::container::hash_map<int, int> > hm;
    ...
    namespace {
        ...
        class hash_map_add_item : public flare::container::hash_map<int, int> {
        public:
