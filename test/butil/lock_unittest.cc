...
// Basic test to make sure that Acquire()/Release()/Try() don't crash ----------
    ...
    public:
        explicit BasicLockTestThread(flare::base::Lock *lock) : lock_(lock), acquired_(0) {}
...
// Test that Try() works as expected -------------------------------------------
    ...
    public:
        explicit TryLockTestThread(flare::base::Lock *lock) : lock_(lock), got_lock_(false) {}
...
// Tests that locks actually exclude -------------------------------------------
    ...
    public:
        MutexLockTestThread(flare::base::Lock *lock, int *value) : lock_(lock), value_(value) {}

        // Static helper which can also be called from the main thread.
        static void DoStuff(flare::base::Lock *lock, int *value) {
            for (int i = 0; i < 40; i++) {
                lock->Acquire();
                int v = *value;
