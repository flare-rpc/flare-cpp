...
// all synchronized access.
        ...
        public:
            ...
            flare::base::ConditionVariable *work_is_available();

            flare::base::ConditionVariable *all_threads_have_ids();

            flare::base::ConditionVariable *no_more_tasks();
        ...
        private:
            ...
            flare::base::ConditionVariable work_is_available_;  // To tell threads there is work.

            // Conditions to notify the controlling process (if it is interested).
            flare::base::ConditionVariable all_threads_have_ids_;  // All threads are running.
            flare::base::ConditionVariable no_more_tasks_;  // Task count is zero.
...
//------------------------------------------------------------------------------
        ...
        TEST_F(ConditionVariableTest, StartupShutdownTest) {
            ...
            {
                flare::base::ConditionVariable cv1(&lock);
            }  // Call for cv1 destruction.

            // Exercise with at least a few waits.
            flare::base::ConditionVariable cv(&lock);
        ...
        void ALLOW_UNUSED BackInTime(flare::base::Lock *lock) {
            flare::base::AutoLock auto_lock(*lock);
...
// Test serial task servicing, as well as two parallel task servicing methods.
        TEST_F(ConditionVariableTest, MAYBE_MultiThreadConsumerTest) {
            ...
            {
                flare::base::AutoLock auto_lock(*queue.lock());
        ...
        TEST_F(ConditionVariableTest, LargeFastTaskTest) {
            ...
            flare::base::Lock private_lock;  // Used locally for master to wait.
            flare::base::AutoLock private_held_lock(private_lock);
            flare::base::ConditionVariable private_cv(&private_lock);
...
// lock already acquired.
        ...
        flare::base::Lock *WorkQueue::lock() {
            return &lock_;
        }

        flare::base::ConditionVariable *WorkQueue::work_is_available() {
            return &work_is_available_;
        }

        flare::base::ConditionVariable *WorkQueue::all_threads_have_ids() {
            return &all_threads_have_ids_;
        }

        flare::base::ConditionVariable *WorkQueue::no_more_tasks() {
...
// ask about tasks.
        ...
        void WorkQueue::ThreadMain() {
            ...
            flare::base::Lock private_lock;  // Used to waste time on "our work".
            while (1) {  // This is the main consumer loop.
                ...
                if (work_time > TimeDelta::FromMilliseconds(0)) {
                    ...
                    flare::base::AutoLock auto_lock(private_lock);
                    flare::base::ConditionVariable private_cv(&private_lock);
