...
namespace {
    ...
    struct pf_set {
        ...
        void complete() {
            ...
            pf[2].p.finish(flare::expected<int>(1));
            pf[3].p.finish(
                    flare::unexpected(std::make_exception_ptr(std::logic_error(""))));
        }
    };
    ...
    int expected_noop(flare::expected<int>) {
        ++expect_noop_count;
        return 1;
    }

    void expected_noop_fail(flare::expected<int>) { throw std::runtime_error("dead"); }


    flare::expected<int> generate_expected_value(int) { return 3; }

    flare::expected<int> generate_expected_value_fail(int) {
        return flare::unexpected{std::make_exception_ptr(std::runtime_error("yo"))};
    }

    flare::expected<int> generate_expected_value_throw(int) {
        throw std::runtime_error("yo");
    }


    flare::expected<int> te_generate_expected_value(flare::expected<int>) { return 3; }

    flare::expected<int> te_generate_expected_value_fail(flare::expected<int>) {
        return flare::unexpected{std::make_exception_ptr(std::runtime_error("yo"))};
    }

    flare::expected<int> te_generate_expected_value_throw(flare::expected<int>) {
        throw std::runtime_error("yo");
    }
...
TEST(future_int, blank) { flare::future<int> fut; }
...
TEST(future_int, promote_tuple_to_variadic) {
    flare::promise<std::tuple<int, int>> p_t;
    auto f_t = p_t.get_future();

    flare::future<int, int> real_f = flatten(f_t);

    int a = 0;
    int b = 0;

    real_f.finally([&](flare::expected<int> ia, flare::expected<int> ib) {
        a = *ia;
        b = *ib;
    });
...
TEST(future_int, random_timing) {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        flare::future<int> fut = prom.get_future();
