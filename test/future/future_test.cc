...
namespace flare {
    ...
    static_assert(!std::is_copy_constructible_v<flare::future_internal::boxed<MoveOnlyType>>);
    static_assert(!std::is_copy_assignable_v<flare::future_internal::boxed<MoveOnlyType>>);
    static_assert(std::is_move_constructible_v<flare::future_internal::boxed<MoveOnlyType>>);
    static_assert(std::is_move_assignable_v<flare::future_internal::boxed<MoveOnlyType>>);
...
// no argument.
    ...
    TEST(Usage, Fork) {
        future<int> rf(1);
        auto forked = flare::future_internal::fork(&rf);  // (Will be) satisfied with the same value as
        // of `rf`.
        bool cont_called = false;
    ...
    TEST(Usage, ForkVoid) {
        ...
        auto forked = flare::future_internal::fork(&rf);
        bool cont_called = false;
...
// > concurrently.
    ...
    TEST(FutureV2Test, ConcurrentFork) {
        for (int i = 0; i != 100000; ++i) {
            ...
            auto t = std::thread([&] {
                l.Countdown();
                flare::future_internal::fork(&fs).then([&](auto &&...) { ++x; });
            });
    ...
    TEST(FutureV2Test, ExecutorTest) {
        ...
        auto test = [] {
            ...
            for (int i = 0; i != 10000; ++i) {
                f = std::move(f).then([&] { ASSERT_GT(posted_jobs, 0Ul); });
            }
            p.set_value();
        ...
        ASSERT_EQ(posted_jobs, 10 * 10000 + 10UL);
