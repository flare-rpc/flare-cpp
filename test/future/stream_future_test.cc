...
#include "flare/future/stream_future.h"
#include <array>
#include <queue>
#include <random>
...
TEST(future_stream, ignored_promise) {
    flare::stream_promise<int> prom;

    (void) prom;
}

TEST(future_stream, ignored_future) {
    flare::stream_future<int> prom;
...
TEST(future_stream, forgotten_promise) {
    ...
    ASSERT_THROW(all_done.get(), flare::unfull_filled_promise);
}
...
TEST(future_stream, simple_stream) {
    ...
    fut.for_each([&](int v) { total += v; }).finally([&](flare::expected<void>) {
        total = -1;
    });
...
TEST(future_stream, string_stream) {
    flare::stream_promise <std::string> prom;
    auto fut = prom.get_future();
...
TEST(future_stream, dynamic_mem_stream) {
    flare::stream_promise <std::unique_ptr<int>> prom;
    auto fut = prom.get_future();
...
TEST(future_stream, dynamic_mem_dropped) {
    ...
    ASSERT_THROW(done.get(), flare::unfull_filled_promise);
}
...
TEST(future_stream, uncompleted_stream) {
    ...
    {
        flare::stream_promise<int> destroyer = std::move(prom);
        ASSERT_EQ(total, 0);
        destroyer.push(1);
        ASSERT_EQ(total, 1);
        destroyer.push(2);
...
TEST(future_stream, stream_to_queue) {
    ...
    fut.for_each(queue, [&](int v) { total += v; }).finally([&](flare::expected<void>) {
        all_done = true;
    });
