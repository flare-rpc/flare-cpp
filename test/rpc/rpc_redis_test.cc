...
#define REDIS_SERVER_PORT "6479"
    ...
    public:
        ...
        flare::rpc::RedisCommandHandlerResult OnBatched(const std::vector<std::string_view> &args,
            ...
            if (_batched_command.empty() && flush_batched) {
                if (args[0] == "set") {
                    DoSet(flare::as_string(args[1]), flare::as_string(args[2]), output);
                } else if (args[0] == "get") {
                    DoGet(flare::as_string(args[1]), output);
                }
                return flare::rpc::REDIS_CMD_HANDLED;
            }
            std::vector<std::string> comm;
            for (int i = 0; i < (int) args.size(); ++i) {
                comm.push_back(flare::as_string(args[i]));
            }
    ...
    public:
        ...
        flare::rpc::RedisCommandHandlerResult Run(const std::vector<std::string_view> &args,
            ...
            int64_t value;
            s_mutex.lock();
            value = ++int_map[flare::as_string(args[1])];
            s_mutex.unlock();
    ...
    TEST_F(RedisTest, server_sanity) {
        ...
        ASSERT_TRUE(flare::starts_with(response.reply(6).error_message(), "ERR unknown command"));
