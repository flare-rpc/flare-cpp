...
public:
    ...
    void OnPlay(const flare::rpc::RtmpPlayOptions& opt,
        ...
        if (_sleep_ms > 0) {
            FLARE_LOG(INFO) << "Sleep " << _sleep_ms
    ...
    void OnStop() {
        FLARE_LOG(INFO) << "OnStop of PlayingDummyStream=" << this;
...
void PlayingDummyStream::SendData() {
    FLARE_LOG(INFO) << "Enter SendData of PlayingDummyStream=" << this;
    ...
    for (int i = 0; !fiber_stopped(fiber_self()); ++i) {
        ...
        vmsg.data.clear();
        vmsg.data.append(flare::string_format("video_{}(ms_id={})",
                                             i, stream_id()));
        ...
        amsg.data.clear();
        amsg.data.append(flare::string_format("audio_{}(ms_id={})",
                                             i, stream_id()));
    ...
    FLARE_LOG(INFO) << "Quit SendData of PlayingDummyStream=" << this;
}
...
public:
    ...
    void OnStop() {
        FLARE_LOG(INFO) << "OnStop of PublishStream=" << this;
        ++_called_on_stop;
    }
    void OnVideoMessage(flare::rpc::RtmpVideoMessage* msg) {
        ++_nvideomsg;
...
TEST(RtmpTest, successfully_play_streams) {
    ...
    for (int i = 0; i < NSTREAM; ++i) {
        ...
        opt.play_name = flare::string_format("play_name_{}", i);
        //opt.publish_name = flare::string_format("pub_name_{}", i);
    ...
    FLARE_LOG(INFO) << "Quiting program...";
}
...
TEST(RtmpTest, successfully_publish_streams) {
    ...
    for (int i = 0; i < REP; ++i) {
        ...
        vmsg.frame_type = flare::rpc::FLV_VIDEO_FRAME_KEYFRAME;
        vmsg.codec = flare::rpc::FLV_VIDEO_AVC;
        vmsg.data.append(flare::string_format("video_{}", i));
        ...
        amsg.bits = flare::rpc::FLV_SOUND_16BIT;
        amsg.type = flare::rpc::FLV_SOUND_STEREO;
        amsg.data.append(flare::string_format("audio_{}", i));
...
TEST(RtmpTest, retrying_stream) {
    ...
    for (int i = 0; i < NSTREAM; ++i) {
        ...
        flare::rpc::RtmpRetryingClientStreamOptions opt;
        opt.play_name = flare::string_format("name_{}", i);
        flare::rpc::SubStreamCreator* sc = new RtmpSubStreamCreator(&rtmp_client);
        cstreams[i]->Init(sc, opt);
    }
    sleep(3);
    FLARE_LOG(INFO) << "Stopping server";
    server.Stop(0);
    server.Join();
    FLARE_LOG(INFO) << "Stopped server and sleep for a while";
    sleep(3);
    ASSERT_EQ(0, server.Start(8576, &server_opt));
    sleep(3);
    for (int i = 0; i < NSTREAM; ++i) {
