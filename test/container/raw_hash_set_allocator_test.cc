...
TEST_F(PropagateOnAll, Empty) { EXPECT_EQ(0ul, a1.num_allocs()); }

TEST_F(PropagateOnAll, InsertAllocates) {
  auto it = t1.insert(0).first;
  EXPECT_EQ(1ul, a1.num_allocs());
  EXPECT_EQ(0ul, it->num_moves());
  EXPECT_EQ(0ul, it->num_copies());
}
...
TEST_F(PropagateOnAll, RehashMoves) {
  ...
  EXPECT_EQ(2ul, a1.num_allocs());
  it = t1.find(0);
  EXPECT_EQ(1ul, it->num_moves());
  EXPECT_EQ(0ul, it->num_copies());
}
...
TEST_F(NoPropagateOnCopy, CopyConstructor) {
  ...
  Table u(t1);
  EXPECT_EQ(1ul, a1.num_allocs());
  EXPECT_EQ(1ul, u.get_allocator().num_allocs());
  EXPECT_EQ(0ul, it->num_moves());
  EXPECT_EQ(1ul, it->num_copies());
}
