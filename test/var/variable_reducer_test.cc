...
namespace {
    ...
    static long start_perf_test_with_atomic(size_t num_thread) {
        ...
        flare::thread threads[num_thread];
        for (size_t i = 0; i < num_thread; ++i) {
            flare::thread th("", [&] {
                add_atomic((void *) &counter);
            });
    ...
    TEST_F(ReducerTest, non_primitive_mt) {
        ...
        for (size_t i = 0; i < FLARE_ARRAY_SIZE(th); ++i) {
            ASSERT_EQ(appended_count[th[i].native_handler()], got_count[th[i].native_handler()]);
        }
    }
